...
/**
 * Standard Parashari D20 calculation.
 *
 * Like D16, the starting sign for counting depends on the NATURE
 * (movable / fixed / dual) of the D1 sign occupied, but the
 * fixed/dual starting signs are swapped relative to D16:
 *   movable -> start from Aries (1)
 *   fixed   -> start from Sagittarius (9)
 *   dual    -> start from Leo (5)
 */
function calculateD20Position(longitude) {
   ...
   * 1.500° - 3.000°  => index 1
   * ...
   * 28.500° - 30.00° => index 19
   */
  let partIndex = Math.floor(degreeInSign / PART_SIZE);

  partIndex = Math.max(0, Math.min(19, partIndex));
  ...
  /**
   * D20 sign = starting sign (based on D1 sign's nature)
   * moved forward by partIndex.
   */
  const d20SignId = ((startSignId - 1 + partIndex) % 12) + 1;

  /**
   * Position inside the 1.5° D20 subdivision.
   */
  const degreeInsidePart = degreeInSign - (partIndex * PART_SIZE);

  /**
   * Convert 0°-1.5° into 0°-30' (scale factor = 20)
   */
  const d20DegreeInSign = degreeInsidePart * 20;
  ...
  return {
    ...
    signName: SIGN_DATA[d20SignId]?.name || null,

    signHindi: SIGN_DATA[d20SignId]?.hindi || null,

    d20DegreeInSign: Number(d20DegreeInSign.toFixed(4))
  };
}

function buildHouses(d20AscendantSignId, planetaryPositions) {
  const houses = {};

  for (let house = 1; house <= 12; house++) {
    const signId = ((d20AscendantSignId - 1 + (house - 1)) % 12) + 1;
  ...
  for (const [planet, position] of Object.entries(planetaryPositions)) {
    ...
    const house = ((position.signId - d20AscendantSignId + 12) % 12) + 1;
    ...
    houses[String(house)].occupants.push({
      ...
      d20DegreeInSign: position.d20DegreeInSign,
...
function generateD20Chart(planetLongitudes) {
   ...
   * Ascendant
   */
  const ascendant = calculateD20Position(planetLongitudes.Ascendant);
  ...
  for (const planet of PLANETS) {
    ...
    planetaryPositions[planet] = calculateD20Position(
  ...
  const d20Ascendant = {
    ...
    degreeInSign: ascendant.d20DegreeInSign
  };

  const houses = buildHouses(d20Ascendant.signId, planetaryPositions);

  return {
    chartType: 'D20',

    chartName: 'Vimshamsha Chart',
