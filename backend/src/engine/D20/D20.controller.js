 ...
 *
 * NOTE:
 * This assumes the D20 folder sits at the same directory level
 * as the D1 folder, exactly like:
 *
 *   /charts
 *     /D1
 *     /D16
...
const D20Service = require('./D20.service');
 ...
 * ============================================================
 *
 * D20 planetary calculation independently nahi karta.
 ...
 * ============================================================
 */
async function getD20Chart(req, res) {
  try {
    ...
    const result = D20Service.processD20Chart({
    ...
    console.error('D20 Controller Error:', error);

    return res.status(400).json({
      success: false,
      error: {
        message: error?.message || 'Unable to generate D20 chart.'
      }
    });
  }
}
 ...
 * ============================================================
 */
async function getD20FromPlanetLongitudes(req, res) {
  try {
    const body = req?.body || {};
    ...
    return res.json({
      ...
      data: {
        ...
        d1LongitudesUsed: D20Service.validatePlanetLongitudes(planetLongitudes),
        ...result
      }
    });
  } catch (error) {
    console.error('D20 From Longitudes Error:', error);
