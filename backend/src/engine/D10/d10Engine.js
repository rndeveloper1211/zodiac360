/**
 * D10 Core Orchestrator
 * ---------------------
 * D1 raw -> D10 calculation -> 10th house/Saturn/Sun/career-field analysis -> timing.
 */

const { generateD10Chart } = require('./D10chartengine');
const {
  computeMahadashaSequence,
  buildCompactD10Response
} = require('./D10advancedrules');
const { processD10Interpretation } = require('./d10Interpreter');

function unwrapD1Data(input) {
  if (!input || typeof input !== 'object') {
    throw new Error('D1 input data is required.');
  }
  if (input.data?.raw) return input.data.raw;
  if (input.data?.lagna && input.data?.grahas) return input.data;
  return input;
}

function extractMoonTotalDegree(d1RawData) {
  const moon = d1RawData?.grahas?.Moon || d1RawData?.planets?.Moon;
  if (!moon) return null;
  if (Number.isFinite(Number(moon.totalDegree))) return Number(moon.totalDegree);
  if (Number.isFinite(Number(moon.signId)) && Number.isFinite(Number(moon.degreeInSign))) {
    return (Number(moon.signId) - 1) * 30 + Number(moon.degreeInSign);
  }
  return null;
}

function extractBirthDate(d1RawData) {
  return d1RawData?.meta?.inputDate ||
    d1RawData?.meta?.dob ||
    d1RawData?.meta?.birthDate ||
    null;
}

function processD10Chart(inputD1Data, options = {}) {
  try {
    const d1RawData = unwrapD1Data(inputD1Data);
    if (!d1RawData?.lagna || !d1RawData?.grahas) {
      throw new Error('Valid D1 raw data with lagna and grahas is required for D10 calculation.');
    }

    const d10ChartData = generateD10Chart(d1RawData);
    const moonTotalDegree = extractMoonTotalDegree(d1RawData);
    const birthDateISO = extractBirthDate(d1RawData);

    let mahadashaSequence = null;
    if (moonTotalDegree !== null && birthDateISO) {
      try {
        mahadashaSequence = computeMahadashaSequence(moonTotalDegree, birthDateISO);
      } catch (error) {
        console.warn('D10 Vimshottari computation skipped:', error.message);
      }
    }

    const interpretation = processD10Interpretation(
      d10ChartData,
      moonTotalDegree,
      birthDateISO,
      mahadashaSequence,
      {
        d1RawData,
        transits: options.transits || d1RawData.transits || null,
        asOfDate: options.asOfDate || new Date(),
        maxTimingYears: options.maxTimingYears || 20
      }
    );

    const publicAnalysis = buildCompactD10Response(interpretation);

    const compactChart = {
      chartType: d10ChartData.chartType,
      chartName: d10ChartData.chartName,
      calculationSystem: d10ChartData.calculationSystem,
      meta: d10ChartData.meta,
      lagna: d10ChartData.lagna,
      tenthLordName: d10ChartData.tenthLordName,
      houseSigns: d10ChartData.houseSigns,
      houseLords: d10ChartData.houseLords
    };

    const result = {
      success: true,
      chartType: 'D10',
      data: {
        chart: compactChart,
        analysis: publicAnalysis
      }
    };

    if (options.includeRaw === true || options.debug === true) {
      result.data.raw = d10ChartData;
      result.data.debugAnalysis = interpretation;
    }

    return result;
  } catch (error) {
    console.error('Error in processD10Chart:', error);
    throw error;
  }
}

module.exports = {
  processD10Chart,
  unwrapD1Data,
  extractMoonTotalDegree,
  extractBirthDate
};
