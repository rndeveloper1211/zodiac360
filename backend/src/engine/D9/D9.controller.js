...
console.log('\n================== [1. GENERATED RAW DATA FOR D9] ==================');
    console.log(JSON.stringify(rawReport, null, 2));
    const rawData = rawReport.data ? (rawReport.data.raw || rawReport.data) : rawReport;
    const d9AnalysisReport = processD9Chart(rawData);

    return res.status(200).json({
      success: true,
      data: d9AnalysisReport.data
    });
  } catch (error) {
    console.error('Error in D9 Controller (getD9):', error);
...
function analyzeExistingD9(req, res) {
  try {
    const payload = req.body;
    const rawData = payload.data ? (payload.data.raw || payload.data) : payload;
    const interpretation = processD9Chart(rawData);
    ...
    console.error('Error in D9 Controller (analyzeExistingD9):', error);
    return res.status(500).json({
      success: false,
      error: error.message
    });
