...
function getD45(req, res) {
  try {
    ...
    const report = generateD45Report({
      date,
      time,
      lat: latitude,
      lon: longitude,
    ...
    console.error('[D45] Controller Error:', error);
    return res.status(500).json({ success: false, error: error.message });
  }
}

function analyzeExistingD45(req, res) {
  try {
    const body = req.body || {};
    ...
    const chart = processExistingD1ToD45(d1Chart);
    return res.status(200).json({ success: true, data: chart });
  } catch (error) {
    console.error('[D45] Analyze Error:', error);
    return res.status(500).json({ success: false, error: error.message });
  }
}

module.exports = { getD45, analyzeExistingD45 };
